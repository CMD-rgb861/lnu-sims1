...
class StudentAccountController extends Controller
{
    ...
    public function studentProfileIndex()
    {
        ...
        return view('pages.employees.user_management.student_accounts_layout', $data, compact('regions', 'provinces', 'municipalities', 'barangays'));
    }
    ...
    public function createStudentProfileInfo(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'id_number' => 'required|string|min:7|max:10',
            'first_name'=> 'required|string|max:255',
        ...
        try{
            ...
            // Log user activity
            UserLogsProvider::log('created a new student profile for: ' . $studentAccount->first_name . ' ' . $studentAccount->last_name);
            ...
            if ($request->ajax()) {
                ...
                // If it's a normal form submission
                return redirect()->back()->with('toast', [
                    'text' => 'Failed updating personal information!',
                    'type' => 'error',
                ]);
            }
        }
    ...
    public function uploadStudentPictures(Request $request)
    {
        ...
        $request->validate([
            ...
            'edit_id' => 'required|integer|exists:student_profiles,student_account_id'
        ]);

        $studentProfile = StudentProfile::where('student_account_id', $request->edit_id)->firstOrFail();
        $studentNumber = StudentAccount::findOrFail($request->edit_id);
        ...
        try{
            ...
            if ($request->hasFile('profile_pic')) {
                ...
                $customName = "{$studentNumber->id_number}" . '.' . $extension;
            ...
            DB::rollBack();
            return redirect()->back()->with('toast', [
                'text' => 'Failed uploading photo.' . $e->getMessage(),
                'type' => 'error',
            ]);
        }
